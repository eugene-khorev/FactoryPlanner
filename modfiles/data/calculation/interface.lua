...
local function start_base_solver(player, subfactory)
...
local function start_matrix_solver(player, subfactory)
    ...
    if num_rows ~= num_cols then
        modal_dialog.enter(player, {type="matrix_solver", submit=true, modal_data=modal_data})
    ...
    elseif num_rows == num_cols then
        ...
        local subfactory_data = calculation.interface.get_subfactory_data(player, subfactory)
        matrix_solver.run_matrix_solver(player, subfactory_data, subfactory_data.matrix_free_items)
        player_table.active_subfactory = nil
    end
end
...
function calculation.update(player, subfactory, refresh)
    if subfactory ~= nil and subfactory.valid then
        if subfactory.matrix_free_items ~= nil then
            start_matrix_solver(player, subfactory)
        else
            start_base_solver(player, subfactory)
        end
    end
